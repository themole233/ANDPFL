...
if __name__ == '__main__':
    ...
    for name, parms in net_glob.named_parameters():	
        ...
        # theta_glob[name] = torch.zeros_like(parms.data).to(args.device)
        # print(name)
        ...
        for idx in idxs_users: # 一次迭代中选取的客户数目
            ...
            for iter in range(args.local_ep):
                ...
                for batch_idx, (images, labels) in enumerate(ldr_train):
                    ...
                    # optimizer.step() # 根据当前参数上的梯度更新参数

                    # for name in params:
                    #     w[name] += copy.deepcopy(params[name].grad)
                ...
                if iter == args.local_ep-1:
                    ...
                    for name, parms in net_local.named_parameters():                	
                        w[name] = parms.grad
                        # parms.grad = local_disturb(w[name])
                else:
                    optimizer.step()
                    optimizer.zero_grad()
            ...
            for i in range(len(var_expe_w)): # 对每一层进行运算
                ...
                if 1==1:
                    ...
                    # if 1==1:
                        ...
                        # w[idx_key] = w[idx_key] + torch.normal(0, C*sigma_star, w[idx_key].size())
                        # w[idx_key] = w[idx_key] + torch.normal(10, C*sigma_star*10, w[idx_key].size())
                        # w[idx_key] = w[idx_key] + torch.normal(1e+10, 1e+10, w[idx_key].size())
                        ...
                        # w[idx_key] = w_clip[idx_key] + torch.normal(0, sigma[idx_key])
                        # w[idx_key] = w_clip[idx_key] + torch.normal(10, sigma[idx_key]*10)
                        # w[idx_key] = w[idx_key] + torch.normal(1e+10, 1e+10, w[idx_key].size())
