...
if __name__ == '__main__':
    ...
    print(args.an)
    def parameter_perturb(delta_w,expe_w_star,C=args.lcf,sigma_0=args.nl,gamma_prime=args.gamma_star,beta=args.beta,G=1e-6):
        key = list(delta_w.keys())
        s = dict()
        sigma = dict()
        var_expe_w = dict()
        ...
        for k, v in expe_w_star_sqrt.items():
        ...
        for idx_key in key:
            if var_expe_w[idx_key] <= G or not (args.an):
                ...
                delta_w[idx_key] = delta_w[idx_key] / max(1, float(torch.norm(delta_w[idx_key], p=2)) / C)
                delta_w[idx_key].view(delta_w_shape)
                delta_w[idx_key] = delta_w[idx_key] + torch.normal(0, C * sigma_0, delta_w[idx_key].size())
    ...
    # delta_locals = torch.from_numpy(delta_locals)

    expe_w_locals = [copy.deepcopy(expe_w) for i in range(args.num_users)]
    expe_w_sqrt_locals = [copy.deepcopy(expe_w_sqrt) for i in range(args.num_users)]
    ...
    delta_repro = []
    delta_local_repro = []
    acc_test_repro=torch.empty(0)
    acc_train_repro=torch.empty(0)
    ...
    for iter in range(args.epochs):
        ...
        # idxs_users = torch.from_numpy(idxs_users)
        # print(idxs_users)
        ...
        for idx in range(args.num_users): # 所有客户并行训练
                ...
                for iterr in range(local_ep):   
                    ...
                    for batch_idx, (images, labels) in enumerate(ldr_train):
                        ...
                        for k,v in theta_local.items():
                            ...
                            R2 = R2+0.05*torch.sum((theta_local[k]-theta_glob[k]).sqrt())/2
                        ...
                        if aa == int(len(dict_users[idx])/batch_size) and iterr == local_ep-1:
                            # optimizer.zero_grad()
                            break
                        optimizer.step() #更新参数
            ...
            #把参数存下来
            delta_ww = torch.empty(0).to(args.device)
            if iter == 0 and idx==idxs_users[0] and args.save==True:
                for idx_keyy in key:
                    # delta_ww.append((delta_w[idx_keyy].numpy()).flatten())
                    # delta_ww.append(torch.flatten(delta_w[idx_keyy]))
                    delta_ww = torch.cat((delta_ww,torch.flatten(delta_w[idx_keyy])))
                # delta_ww = np.concatenate(delta_ww)
                delta_ww = torch.flatten(delta_ww)
                np.savetxt("./savedata/delta_w_round{}.txt".format(iter),delta_ww.numpy())
            ...
            sigmaa = torch.empty(0).to(args.device)
            ss = torch.empty(0).to(args.device)
            for idx_key in key: # 对每一层进行运算
                ...
                if var_expe_w[idx_key] <= G or not(args.an):
                    ...
                    if iter==0 and idx==idxs_users[0] and args.save==True:
                        ...
                        ss = torch.cat((ss,torch.flatten(s[idx_key])))
                        sigmaa = torch.cat((sigmaa,torch.flatten(sigma[idx_key])))
            ...
            if iter==0 and idx==idxs_users[0] and args.save==True:
                ...
                ss = torch.flatten(ss)
                sigmaa = torch.flatten(sigmaa)
                np.savetxt("./savedata/prior_delta_w_round{}.txt".format(iter),(ss/beta).numpy())
                np.savetxt("./savedata/sigma_round{}.txt".format(iter),sigmaa.numpy())
                # sys.exit()
                    ...
                    if alpha[i]==2:
                        ...
                        for j in range(3, alpha[i]+1):
                            try:
                                ...
                                calc_sum = float('inf')
                    log_delta[i] = -(epsilon-(L[idx]*(aa*local_ep)/(alpha[i]-1))*math.log(1+comb(alpha[i],2)*q**2*x+4*calc_sum))*(alpha[i]-1)
        ...
        if (iter+1)%1 == 0:
            ...
            # delta_repro.append(delta) 
            # print(delta)
            # delta_repro = torch.cat((delta_repro,delta))
            # delta_local_repro = torch.cat((delta_local_repro,delta_local))
            net_glob.eval()
            ...
            # print(final_acc_test)
            acc_train_repro = torch.cat((acc_train_repro,final_acc_train.view(1)))
            # acc_train_repro.append(final_acc_train)
            # acc_test_repro.append(final_acc_test)
            acc_test_repro = torch.cat((acc_test_repro,final_acc_test.view(1)))
    ...
    np.savetxt("./savedata/round_epsilon{}_frac{}_lr{}_nl{}_beta{}_gamma{}_ls{}_lepoch{:.2f}_an{}_iid{}.txt".format(args.epsilon, args.frac, args.lr, args.nl, args.beta, args.gamma_star, args.ls, args.local_ep, args.an, args.iid),x)
    y3 = np.array([i * 10000 for i in delta_repro])
    np.savetxt("./savedata/downlink_epsilon{}_frac{}_lr{}_nl{}_beta{}_gamma{}_ls{}_lepoch{:.2f}_an{}_iid{}.txt".format(args.epsilon, args.frac, args.lr, args.nl, args.beta, args.gamma_star, args.ls, args.local_ep, args.an, args.iid),y3)

    y4 = np.array([i * 10000 for i in delta_local_repro])
    np.savetxt("./savedata/upload_epsilon{}_frac{}_lr{}_nl{}_beta{}_gamma{}_ls{}_lepoch{:.2f}_an{}_iid{}.txt".format(args.epsilon, args.frac, args.lr, args.nl, args.beta, args.gamma_star, args.ls, args.local_ep, args.an, args.iid),y4)

    y1 = np.array(acc_test_repro)
    np.savetxt("./savedata/testing_epsilon{}_frac{}_lr{}_nl{}_beta{}_gamma{}_ls{}_lepoch{:.2f}_an{}_iid{}.txt".format(args.epsilon, args.frac, args.lr, args.nl, args.beta, args.gamma_star, args.ls, args.local_ep, args.an, args.iid),y1.numpy())
    
    y2 = np.array(acc_train_repro)
    np.savetxt("./savedata/training_epsilon{}_frac{}_lr{}_nl{}_beta{}_gamma{}_ls{}_lepoch{:.2f}_an{}_iid{}.txt".format(args.epsilon, args.frac, args.lr, args.nl, args.beta, args.gamma_star, args.ls, args.local_ep, args.an, args.iid),y2.numpy())
