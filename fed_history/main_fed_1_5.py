...
# os.exit() 
    ...
    for iter in range(args.epochs):
        ...
        for idx in range(args.num_users): # 所有客户并行训练
            ...
            net_local.train()
            lot_size = args.ls
            batch_size = int(0.01*len(dict_users[idx])) 
            ...
            if idx in idxs_users:
                ...
                for i in range(63):
                    ...
                    log_delta[i] = -(epsilon-(L[idx]/(alpha[i]-1))*math.log(1+comb(alpha[i],2)*q**2*x+4*calc_sum))*(alpha[i]-1)
    ...
    x = range(len(delta_repro))
    y3 = [i * 10000 for i in delta_repro]
    y4 = [i * 10000 for i in delta_local_repro]
    y1 = acc_test_repro
    y2 = acc_train_repro
