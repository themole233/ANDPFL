...
for k_iter in range(args.epochs):
    ...
    for idx in range(args.num_users):  # 所有客户并行训练
        ...
        if args.local_ep >= 1:
            ...
            for iterr in range(local_ep):
                ...
                for batch_idx, (images, labels) in enumerate(ldr_train):
                    ...
                    theta_local = (net_local.state_dict()).copy()
                    R2 = 0
                    ...
                    delta_w = parameter_perturb(delta_w=delta_w,expe_w_star=expe_w_star)
        ...
        if idx in Z_k_idxs_users:
            ...
            q = aa*lot_size / n_Z_k_number_data_epoch
                ...
                if alpha[i] == 2:
                    ...
                    for j in range(3, alpha[i] + 1):
                        try:
                            ...
                            calc_sum = float('inf')
                log_delta[i] = -(epsilon - (L[idx]  / (alpha[i] - 1)) * math.log(
...
y4 = np.arange(len(delta_local_i)) * 10000
