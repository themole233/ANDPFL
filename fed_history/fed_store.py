...
for k_iter in range(args.rounds):
    ...
    for idx in range(args.num_users):  # 所有客户并行训练
        ...
        ## 参数存储
        w_store = torch.empty(1)
        if idx == 0 and k_iter == 2:
            for k, v in w_local.items():
                w_store = torch.cat((w_store, (v-args.lr*delta_w[k]).flatten()), 0)
        ...
        # delta_w_store = torch.empty(1)
        s_store = torch.empty(1)
        # delta_w_changed_store = torch.empty(1)
        # sigma_store = torch.empty(1)
        ...
        for idx_key in key:  # 对每一层进行运算
            ...
            if var_expe_w[idx_key] <= G or not (args.an):
                ...
                delta_w_shape = delta_w[idx_key].shape  # 保留size

                # if idx == 0 and k_iter == 2:
                #     delta_w_store = torch.cat((delta_w_store, delta_w[idx_key].flatten()), 0)
                #     s_store = torch.cat((s_store, s[idx_key].flatten()), 0)

                delta_w[idx_key].view(delta_w_shape)  # 恢复size

                if idx == 0 and k_iter == 2:
                    s_store = torch.cat((s_store, (w_local[idx_key]-args.lr*s[idx_key]).flatten()), 0)
                ...
                # if idx == 0 and k_iter == 2:
                #     delta_w_changed_store = torch.cat((delta_w_changed_store, delta_w[idx_key].flatten()), 0)

                # delta_w[idx_key].view(delta_w_shape)

        if idx == 0 and k_iter == 2:
            # np.savetxt('./savedata/delta_w_rounds{}.txt'.format(k_iter), delta_w_store.numpy())
            # np.savetxt('./savedata/s_gamma{}.txt'.format(args.gamma_star), s_store.numpy())
            # np.savetxt('./savedata/delta_w_changed_gamma{}.txt'.format(args.gamma_star), delta_w_changed_store.numpy())
            np.savetxt('./savedata/s_rounds{}.txt'.format(k_iter), s_store.numpy())
            np.savetxt('./savedata/w_rounds{}.txt'.format(k_iter), w_store.numpy())
