...
for k_iter in range(args.rounds):
    ...
    for idx in range(args.num_users):  # 所有客户并行训练
        ...
        for batch_idx, (images, labels) in enumerate(ldr_train):
            ...
            if aa == int(31):
                g_used = g.copy()
                Expectation_g_2_locals_used = Expectation_g_2_locals[idx].copy()
        ...
        for idx_key in key:  # 对每一层进行运算
            ...
            if var_expe_w[idx_key] <= G or not args.an:
                ...
                s[idx_key] = args.gamma_star*torch.abs(w_local_previous[idx_key]-args.lr*g_used[idx_key]/torch.sqrt(0.9*Expectation_g_2_locals_used[idx_key]+0.1*
                ...
                w_local[idx_key] = [clamp(x,y,z) for x,y,z in zip(w_local_0[idx_key]-s[idx_key], w_local[idx_key], w_local_0[idx_key]+s[idx_key])]
